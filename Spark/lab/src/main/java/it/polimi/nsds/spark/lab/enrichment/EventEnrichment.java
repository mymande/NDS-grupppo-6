 ...
 *
 * A static dataset (read from a file) classifies products (associates products
 * to the
 * class they belong to). A stream of products is received from a socket.
 *
 * We want to count the number of products of each class in the stream. To do
 * so, we
 * need to integrate static knowledge (product classification) and streaming
 * data
...
public class EventEnrichment {
        public static void main(String[] args) throws TimeoutException {
                ...
                productClassificationFields
                                .add(DataTypes.createStructField("classification", DataTypes.StringType, false));
                ...
                Dataset<Row> inStreamDF = inStream.toDF("timestamp", "value");
                ...
                // TODO
                inStream.withWatermark("timestamp", "1 hour");

                final StreamingQuery query = inStreamDF
                                .join(productsClassification,
                                                inStreamDF.col("value").equalTo(productsClassification.col("product")))
                                .groupBy(window(col("timestamp"), "30 seconds", "5 seconds"), col("value"),
                                                col("classification"))
                                .count()
                                .writeStream()
                                .outputMode("update")
