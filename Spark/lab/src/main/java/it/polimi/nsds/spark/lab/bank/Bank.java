...
public class Bank {
    ...
    public static void main(String[] args) {
        ...
        final Dataset<Row> negativeAccounts = totWithdrawals
                ...
                .filter(totDeposits.col("sum(amount)").isNull().and(totWithdrawals.col("sum(amount)").gt(0))
        ...
        final Dataset<Row> negwith = totWithdrawals
                .withColumn("sum(amount)", col("sum(amount)").multiply(-1));

        negwith.show();

        final Dataset<Row> balance = totDeposits
                // .withColumn("balance",
        ...
        // "full_outer");
        // .select(totDeposits.col("account").)
        balance.show();
        spark.close();
    }
